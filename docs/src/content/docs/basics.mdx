...
click <PluginIcon name="generate" /> Generate (Shift + Enter). This will ignore
...
To apply one of the results, click its <PluginIcon name="apply" /> Apply button,
...
generate action to <PluginIcon name="refine" /> Refine, also known as _image-to-image_.
...
area. Note how the generate action changes to <PluginIcon name="inpaint-fill" /> Fill
...
<PluginIcon name="control-add" /> Add control layer button. [Read more about Control Layers.](/control-layers)
...
paint or change parameters. Press the <PluginIcon name="play" /> Play button to start
live generation.
...
You can also build custom UI for parametrization! [Read more about Custom Graphs.](/custom-graphs)
