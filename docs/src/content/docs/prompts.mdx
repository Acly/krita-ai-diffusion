...
Everything following a `#` character is ignored, until the end of the line.
This is useful for temporarily disabling parts of the prompt.
...
a sunny day
```
The example would send the following text to the model:
`a songbird on a tree branch a sunny day`

### Wildcards

You can use wildcards to randomly select words or phrases from a list. This is
useful for initial exploration and experiments. Wildcards are enclosed in curly
braces like this: `{option 1|option 2|option 3}`. You can have as many options as you
like. Only one of them is used to generate the image.

Example: `a {red|blue} car is parked {on the street|underneath a tree}`

The actual prompt used will be one of:
* `a red car is parked on the street`
* `a red car is parked underneath a tree`
* `a blue car is parked on the street`
* `a blue car is parked underneath a tree`

Simply enqueue multiple generations to get some variety. The results will indicate
which options were selected in the tooltip, and you can copy the "evaluated" prompt
from the image you like best.

<ImageSample src="prompts/wildcards.webp" alt="Wildcard results showing different selected options" scale={1.5} />

<Aside type="tip">
Wildcard evaluation depends on the seed. The same fixed seed will always select the
same options, and generate the same image!
</Aside>
...
Layers that should be used as a reference image for generation can be added to
the text prompt directly. This is equivalent to adding a 
[Control layer](control-layers#reference-images) and
choosing the <PluginIcon name="control-reference" simple /> Reference mode.

The syntax is `<layer:your layer name>`. The name must match one of the image
layers in the current document. Auto-completion is available.

<ImageSample src="prompts/layers.png" alt="Selecting a reference layer in the text prompt" scale={1.5} />

If you are using an [Edit model](edit-models), you can use this to indicate
which image you are referring to. Currently only Flux 2 and Qwen edit models
supports referring to images in text directly:
* Example (Flux 2/Qwen Edit): `Put the cat in <layer:cat photo> next to the dog.`
* Example (other models): `Put the black cat <layer:cat photo> next to the dog.`
...
<Aside type="note">
Chinese models (like Qwen) typically understand both Chinese and English natively.
It is recommended to use Chinese input without translation.
</Aside>
